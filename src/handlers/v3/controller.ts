...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "GlobalProtocolSwapFeePercentageChanged" },
  async ({ event, context }) => {
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "GlobalProtocolYieldFeePercentageChanged" },
  async ({ event, context }) => {
  const chainId = event.chainId;
  const vaultId = makeChainId(chainId, VAULT_ADDRESS);
  const vault = await context.V3Vault.get(vaultId);
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "PoolCreatorSwapFeePercentageChanged" },
  async ({ event, context }) => {
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "PoolCreatorYieldFeePercentageChanged" },
  async ({ event, context }) => {
  const poolId = makeChainId(event.chainId, event.params.pool);
  const pool = await context.V3Pool.get(poolId);
  if (!pool) return;
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "ProtocolSwapFeePercentageChanged" },
  async ({ event, context }) => {
  const poolId = makeChainId(event.chainId, event.params.pool);
  const pool = await context.V3Pool.get(poolId);
  if (!pool) return;
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "ProtocolYieldFeePercentageChanged" },
  async ({ event, context }) => {
  const poolId = makeChainId(event.chainId, event.params.pool);
  const pool = await context.V3Pool.get(poolId);
  if (!pool) return;
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "ProtocolSwapFeeCollected" },
  async ({ event, context }) => {
  const chainId = event.chainId;
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "ProtocolYieldFeeCollected" },
  async ({ event, context }) => {
  const chainId = event.chainId;
  const ptId = getPoolTokenId(chainId, event.params.pool, event.params.token);
  const pt = await context.V3PoolToken.get(ptId);
...
indexer.onEvent(
  { contract: "V3ProtocolFeeController", event: "ProtocolFeesWithdrawn" },
  async ({ event, context }) => {
  const chainId = event.chainId;
  const ptId = getPoolTokenId(chainId, event.params.pool, event.params.token);
  const pt = await context.V3PoolToken.get(ptId);
